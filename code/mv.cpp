...
float mx[100000];
 //glMatrixMode(GL_MODELVIEW);
	if(isFog){
		...
		fogMode = GL_LINEAR;  //initial fog mode
		...
		glFogf (GL_FOG_END, 5.0);  //end distance for linear fog
		   ...
		   //glClearColor(0.5, 0.5, 0.5, 0.5);  // set the background color = fog color
	}
	//glLoadIdentity();
	//TODO point of instability
	/*
	switch(viewType){
		case 'V':
			//glLoadIdentity();
	...
	if (viewType == 'V'){
		...
		switch(direction){
			...
			case 'T': // Tilt Camera
				...
				top += distance * sin(theta2*degToRad);
				bottom += distance * sin(theta2*degToRad);
			...
			case 'A': // Pan Camera
				...
				left += distance * sin(theta3*degToRad);
				right += distance * sin(theta3*degToRad);
			break;
			}
		//glMatrixMode(GL_PROJECTION);
		//gluLookAt(cameraX,cameraY,cameraZ,lookAtX,lookAtY,lookAtZ,upX,upY,upZ);
...
/* =============== Begin  Face   Definition ===============*/
class face
	/* This class should contain informaion collected from each face entry
		we need this class to separate 3 vertix faces from 4 vertix faces
	*/
{
private:
	vertix	vList[10];	// List of Verticies 
	int	vOrder[10];	// for listing purpose
	int	vCount;		// Vertix Count of this this face
	// LAter on add color
public:
	face();
	~face();
	void pushVertix(vertix,int);
	void drawFace();	// draw face individually 
	string dumpFace();	// return an obj entry for this face
	int	getCount();
};

face::face(){
	//cout << "Face Constructor\n";
	vCount = 0;
}

face::~face(){}

void face::pushVertix(vertix v, int VO){ // appends a vertix to to object
	//cout << "Push Vertix \n";
	vList[vCount] = v;
	vOrder[vCount++] = VO;
}
...
void face::drawFace() {
	...
	/*
	glBegin(GL_POLYGON);
	glColor3f(rand(),0.1 ,0);
        for (int idx = 0; idx < vCount; idx++){
		glColor3f(2/(idx+1),1-(1/(idx+1)) ,1);
                glVertex3f(vList[idx].getX(), vList[idx].getY(), vList[idx].getZ() );
	}
	*/
...
private:
        ...
        int     VOCount;
	face	fArray[20000];	// Faces Array
	int	fCount;		// Number of Faces
...
void model::vOrder(string entry){ //Builds Vertix Order Array out of the f entry 
	...
	int vidx=0;
	face f;
	/*face	fArray[100000];	// Faces Array
	int	fCount;		// Number of Faces*/
        if (entry[0] == 'f'){
                cline = entry.c_str();
                line = (char *)cline;
		fCount++;
		fArray[fCount] = f;
		//cout << "New Face #: "<<fCount<<"\n";
                token = strtok_r(line, " ", &ntoken);
                token = strtok_r(NULL, " ", &ntoken); // token has the vertix index 
		//token = strtok_r(NULL, " ", &ntoken); // Skip first entry 
                while (token){
			fArray[fCount].pushVertix(vArray[(int)strtod(token, NULL)],(int)strtod(token, NULL)); //TODO
			/*if(vidx++<3)
		...
		//cout << "ReConstructed Face"  <<fArray[fCount].dumpFace() << "\n";
        }
}
	...
	*/
	...
	switch(key) 
	{
	  ...
	  // Camera Controls
		...
		case 'D': // Dolly +ve == X 
			cam1.translate(tPercesion,0,0);
			break;		
		case 'd': // Dolly -ve
			cam1.translate(-tPercesion,0,0);
			break;
		case 'C': // Crane +ve == Y
			cam1.translate(0,tPercesion,0);
			break;		
		case 'c': // Crane -ve
			cam1.translate(0,-tPercesion,0);
			break;
		case 'I': // Zoom +ve == Z
			cam1.translate(0,0,tPercesion);
			break;		
		case 'i': // Zoom -ve
			cam1.translate(0,0,-tPercesion);
			break;
	  ...
	  // Model Controls 
		...
		case 'x': // Rest Rotation Axis 
			...
			m.resetScale();
			m.setCenter();
			cam1.setCamera(	'v',0, 0, 1, 0, 0,-1, 0, 1, 0);
			cam1.setWindow(-1 ,1 ,-1, 1 ,1 ,100);
			cout << "Reset Scene\n";
			//m.drawModel();
			break;
