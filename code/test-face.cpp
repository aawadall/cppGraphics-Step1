...
char camera::getVt(){ // Get View Type
	switch(viewType)
	{
...
int color=0;
        ...
        for (int idx = 0; idx < VOCount; idx++){
			glColor3f((color++)/VOCount,(color/VOCount)/2 ,(color/VOCount)/3);
...
void model::setCenter(){
	...
	scale(1/fScale);
	//translate(1/(maxX - minX), 1/(maxY - minY), 1/( maxZ - minZ));
	translate(0,0,-2);
	//doNothing();	
	//drawModel();					 
}
	...
	switch(key) 
	{
	  ...
	  // Model Controls 
		...
		case 'x': // Rest Rotation Axis 
			...
			m.resetScale();
			m.setCenter();
			cam1.setCamera(	'v',0, 0, 20, 0, 0,-1, 0, 1, 0);
			cam1.setWindow(-1 ,1 ,-1, 1 ,1 ,100);
			cout << "Reset Scene\n";
			//m.drawModel();
			break;
